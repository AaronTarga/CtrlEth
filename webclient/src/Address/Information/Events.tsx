...
export default function Events({ address, setAlertShown, closeAlert }: EventsProps) {
    ...
    useEffect(() => {
        ...
        apiController.getContractEvents(currentAddress, signal).then((response: ApiResult<ContractEvents>) => {
    ...
    if (loading) {
        ...
        content = (
            <>
                {occurences && occurences.length > 0 && (<SubInfo>
                ...
                { events.events && events.events.length > 0 && (<SubInfo>
