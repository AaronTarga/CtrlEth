...
def str_timestamp_to_date(timestamp):
    ...
    try:
        formatted_date = datetime.datetime.fromtimestamp(
            int_timestamp).strftime('%d-%m-%Y %H:%M:%S')
...
def format_transactions(transactions):
    for transaction in transactions:
        ...
        if (transaction['gasPrice'] == None):
            ...
            transaction['fee'] = Web3.fromWei(
                transaction['gasUsed'] * transaction['gasPrice'], 'ether')
            transaction['gasPrice'] = Web3.fromWei(
                transaction['gasPrice'], 'gwei')
        transaction['value'] = Web3.fromWei(int(transaction['value']), 'ether')
        transaction['timeStamp'] = str_timestamp_to_date(
